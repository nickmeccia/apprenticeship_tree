[:li.slide.three
	[:a.previous {:href "#"}
		[:img.display {:src "./images/hand_left.png"}]
		[:img.hover {:src "./images/hand_left_hover.png"}]]
	[:img {:src "./images/patriarchy_3.png"}]
	[:ul
		[:li#micah_martin
			[:img {:src "./images/micah_martin_photo.png"}]
			[:img.hover {:src "./images/micah_martin_hover.png"}]
			[:img {:src "./images/micah_martin_frame.png"}]
			[:a {:href "http://www.8thlight.com/our-team/micah-martin"} "micah martin"]
			[:figcaption.left
				[:h1 "micah martin"]
				[:p "Here is some information about Micah, such as how he plays guitar and stuff."]
				[:a {:href "http://www.8thlight.com/our-team/micah-martin"} "profile"]
				[:a {:href "http://blog.8thlight.com/micah-martin/archive.html"} "blog"]]]
		[:ol
			[:li#justin_martin
				[:img {:src "./images/justin_martin_photo.png"}]
				[:img.hover {:src "./images/justin_martin_hover.png"}]
				[:img {:src "./images/justin_martin_frame.png"}]
				[:a "justin martin"]
				[:figcaption.left
					[:h1 "justin martin"]
					[:p "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus vitae sem nunc, eget ullamcorper leo. Fusce rhoncus luctus tristique. Vestibulum id turpis est nunc."]
					[:a "profile"]
					[:a {:href "http://blog.8thlight.com/justin-martin/archive.html"} "blog"]]]
			[:li#paul_pagel
				[:img {:src "./images/paul_pagel_photo.png"}]
				[:img.hover {:src "./images/paul_pagel_hover.png"}]
				[:img {:src "./images/paul_pagel_frame.png"}]
				[:a {:href "http://www.8thlight.com/our-team/paul-pagel"} "paul pagel"]
				[:figcaption.right
					[:h1 "paul pagel"]
					[:p "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus vitae sem nunc, eget ullamcorper leo. Fusce rhoncus luctus tristique. Vestibulum id turpis est nunc."]
					[:a {:href "http://www.8thlight.com/our-team/paul-pagel"} "profile"]
					[:a {:href "http://blog.8thlight.com/paul-pagel/archive.html"} "blog"]]]
			[:ol
				[:li#eric_meyer
					[:img {:src "./images/eric_meyer_photo.png"}]
					[:img.hover {:src "./images/eric_meyer_hover.png"}]
					[:img {:src "./images/eric_meyer_frame.png"}]
					[:a {:href "http://www.8thlight.com/our-team/eric-meyer"} "eric meyer"]
					[:figcaption.right
						[:h1 "eric meyer"]
						[:p "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus vitae sem nunc, eget ullamcorper leo. Fusce rhoncus luctus tristique. Vestibulum id turpis est nunc."]
						[:a {:href "http://www.8thlight.com/our-team/eric-meyer"} "profile"]
						[:a {:href "http://blog.8thlight.com/eric-meyer/archive.html"} "blog"]]]
				[:li#brian_pratt
					[:img {:src "./images/brian_pratt_photo.png"}]
					[:img.hover {:src "./images/brian_pratt_hover.png"}]
					[:img {:src "./images/brian_pratt_frame.png"}]
					[:a {:href "http://www.8thlight.com/our-team/brian-pratt"} "brian pratt"]
					[:figcaption.right
						[:h1 "brian pratt"]
						[:p "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus vitae sem nunc, eget ullamcorper leo. Fusce rhoncus luctus tristique. Vestibulum id turpis est nunc."]
						[:a {:href "http://www.8thlight.com/our-team/brian-pratt"} "profile"]
						[:a {:href "http://blog.8thlight.com/brian-pratt/archive.html"} "blog"]]]
				[:li#kevin_liddle
					[:img {:src "./images/kevin_liddle_photo.png"}]
					[:img.hover {:src "./images/kevin_liddle_hover.png"}]
					[:img {:src "./images/kevin_liddle_frame.png"}]
					[:a {:href "http://www.8thlight.com/our-team/kevin-liddle"} "kevin liddle"]
					[:figcaption.right
						[:h1 "kevin liddle"]
						[:p "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus vitae sem nunc, eget ullamcorper leo. Fusce rhoncus luctus tristique. Vestibulum id turpis est nunc."]
						[:a {:href "http://www.8thlight.com/our-team/kevin-liddle"} "profile"]
						[:a {:href "http://blog.8thlight.com/kevin-liddle/archive.html"} "blog"]]]
				[:li#patrick_gombert
					[:img {:src "./images/patrick_gombert_photo.png"}]
					[:img.hover {:src "./images/patrick_gombert_hover.png"}]
					[:img {:src "./images/patrick_gombert_frame.png"}]
					[:a {:href "http://www.8thlight.com/our-team/patrick-gombert"} "patrick gombert"]
					[:figcaption.right
						[:h1 "patrick gombert"]
						[:p "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus vitae sem nunc, eget ullamcorper leo. Fusce rhoncus luctus tristique. Vestibulum id turpis est nunc."]
						[:a {:href "http://www.8thlight.com/our-team/patrick-gombert"} "profile"]
						[:a {:href "http://blog.8thlight.com/patrick-gombert/archive.html"} "blog"]]]]
			[:li#colin_jones
				[:img {:src "./images/colin_jones_photo.png"}]
				[:img.hover {:src "./images/colin_jones_hover.png"}]
				[:img {:src "./images/colin_jones_frame.png"}]
				[:a {:href "http://www.8thlight.com/our-team/colin-jones"} "colin jones"]
				[:figcaption.left
					[:h1 "colin jones"]
					[:p "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus vitae sem nunc, eget ullamcorper leo. Fusce rhoncus luctus tristique. Vestibulum id turpis est nunc."]
					[:a {:href "http://www.8thlight.com/our-team/colin-jones"} "profile"]
					[:a {:href "http://blog.8thlight.com/colin-jones/archive.html"} "blog"]]]
			[:ol
				[:li#mark_grant
					[:img {:src "./images/mark_grant_photo.png"}]
					[:img.hover {:src "./images/mark_grant_hover.png"}]
					[:img {:src "./images/mark_grant_frame.png"}]
					[:a {:href "http://www.8thlight.com/our-team/mark-grant"} "mark grant"]
					[:figcaption.left
						[:h1 "mark grant"]
						[:p "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus vitae sem nunc, eget ullamcorper leo. Fusce rhoncus luctus tristique. Vestibulum id turpis est nunc."]
						[:a {:href "http://www.8thlight.com/our-team/mark-grant"} "profile"]
						[:a {:href "http://blog.8thlight.com/mark-grant/archive.html"} "blog"]]]]
			[:li#steve_kim
				[:img {:src "./images/steve_kim_photo.png"}]
				[:img.hover {:src "./images/steve_kim_hover.png"}]
				[:img {:src "./images/steve_kim_frame.png"}]
				[:a {:href "http://www.8thlight.com/our-team/steve-kim"} "steve kim"]
				[:figcaption.left
					[:h1 "steve kim"]
					[:p "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus vitae sem nunc, eget ullamcorper leo. Fusce rhoncus luctus tristique. Vestibulum id turpis est nunc."]
					[:a {:href "http://www.8thlight.com/our-team/steve-kim"} "profile"]
					[:a {:href "http://blog.8thlight.com/steve-kim/archive.html"} "blog"]]]
			[:li#dave_moore
				[:img {:src "./images/dave_moore_photo.png"}]
				[:img.hover {:src "./images/dave_moore_hover.png"}]
				[:img {:src "./images/dave_moore_frame.png"}]
				[:a {:href "http://www.8thlight.com/our-team/dave-moore"} "dave moore"]
				[:figcaption.left
					[:h1 "dave moore"]
					[:p "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus vitae sem nunc, eget ullamcorper leo. Fusce rhoncus luctus tristique. Vestibulum id turpis est nunc."]
					[:a {:href "http://www.8thlight.com/our-team/dave-moore"} "profile"]
					[:a {:href "http://blog.8thlight.com/dave-moore/archive.html"} "blog"]]]
			[:li#myles_megyesi
				[:img {:src "./images/myles_megyesi_photo.png"}]
				[:img.hover {:src "./images/myles_megyesi_hover.png"}]
				[:img {:src "./images/myles_megyesi_frame.png"}]
				[:a {:href "http://www.8thlight.com/our-team/myles-megyesi"} "myles megyesi"]
				[:figcaption.left
					[:h1 "myles megyesi"]
					[:p "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus vitae sem nunc, eget ullamcorper leo. Fusce rhoncus luctus tristique. Vestibulum id turpis est nunc."]
					[:a {:href "http://www.8thlight.com/our-team/myles-megyesi"} "profile"]
					[:a {:href "http://blog.8thlight.com/myles-megyesi/archive.html"} "blog"]]]
			[:li#wai_lee_chin_feman
				[:img {:src "./images/wai_lee_chin_feman_photo.png"}]
				[:img.hover {:src "./images/wai_lee_chin_feman_hover.png"}]
				[:img {:src "./images/wai_lee_chin_feman_frame.png"}]
				[:a {:href "http://www.8thlight.com/our-team/wai-lee-chin-feman"} "wai lee chin feman"]
				[:figcaption.left
					[:h1 "wai lee chin feman"]
					[:p "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus vitae sem nunc, eget ullamcorper leo. Fusce rhoncus luctus tristique. Vestibulum id turpis est nunc."]
					[:a {:href "http://www.8thlight.com/our-team/wai-lee-chin-feman"} "profile"]
					[:a {:href "http://blog.8thlight.com/wai-lee-chin-feman/archive.html"} "blog"]]]
			[:li#chris_peak
				[:img {:src "./images/chris_peak_photo.png"}]
				[:img.hover {:src "./images/chris_peak_hover.png"}]
				[:img {:src "./images/chris_peak_frame.png"}]
				[:a {:href "http://www.8thlight.com/our-team/chris-peak"} "chris peak"]
				[:figcaption.left
					[:h1 "chris peak"]
					[:p "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus vitae sem nunc, eget ullamcorper leo. Fusce rhoncus luctus tristique. Vestibulum id turpis est nunc."]
					[:a {:href "http://www.8thlight.com/our-team/chris-peak"} "profile"]
					[:a {:href "http://blog.8thlight.com/chris-peak/archive.html"} "blog"]]]]]
	[:a.next {:href "#"}
		[:img.display {:src "./images/hand_right.png"}]
		[:img.hover {:src "./images/hand_right_hover.png"}]]]